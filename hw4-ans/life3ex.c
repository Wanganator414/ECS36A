...
void checkPattern(int argc, char **argv, int *fileRows, int *fileCols)
{
    ...
    char ch;
    patternFile = fopen((argv[argc - 1]), "r");
    if (patternFile == NULL)
    {
        perror(argv[argc - 1]);
        exit(EXIT_FAILURE);
 ...
 * main routine
 */
int main(int argc, char **argv) /*Modified command line input*/
{
    if (argc > 6)
    {
    ...
    /*Print Gen0-maxGen with increment Gen gap*/
    int i = 0;
    for (; i < maxGen; i++)
    {
        if (i % icr == 0)

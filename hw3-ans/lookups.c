...
char **readFile(int argc, char **argv, int fileNum, int *dSize, int *sSize)
{
    ...
    while ((ch = (fgetc(readFile))) != EOF) /*Check amount of rows as newlines*/
    {
        if (ch == '\n')
        {
            ...
            generalSize = (fileNum == 1) ? *dSize : *sSize;
        }
    }
    ...
    if (fileNum == 1) /*Sort dictionary file*/
    {
...
void searchStuff(char **dict, char **search)
{
    ...
    /*printf("Begin checking for matches in dict and search list:\n");*/
    /*printf("Dictionary Size:%d|List Size:%d\n", dSize, sSize);*/
    for (i = 0; i < sSize; i++)
    ...
    avg = (nwcf / sSize) + 0.001;                                                                             /*Cheese to make Grade Scope roudning work correctly*/
